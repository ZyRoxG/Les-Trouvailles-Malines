...
import { TROUVAILLE_CATEGORIES } from "@/lib/categories";
import { useProducts } from "@/hooks/useContent";

export default function Trouvailles() {
  const [params, setParams] = useSearchParams();
  const active = params.get("cat") || "all";
  const { items, loading } = useProducts({ group: "trouvaille" }, 200);

  const filtered = active === "all" ? items : items.filter((p) => p.category === active);
  const activeCat = TROUVAILLE_CATEGORIES.find((c) => c.slug === active);
    ...
    <div>
      <section className="mx-auto max-w-7xl px-5 pt-20 md:px-10 md:pt-28">
        <SectionHeading
          ...
          intro="Des objets malins pour la maison : petits prix, rangement, déco et confort — choisis pour leur utilité et leur douceur."
        />

        <div className="mt-10 flex flex-wrap gap-2.5">
          <FilterChip label="Tout" active={active === "all"} onClick={() => setCat("all")} />
          {TROUVAILLE_CATEGORIES.map((c) => (
  ...
  return (
    <div className="py-24 text-center">
      <p className="font-display text-2xl font-light text-foreground">
        {label ? `Bientôt des trouvailles dans « ${label} »` : "Aucune trouvaille pour l'instant"}
      </p>
      <p className="mx-auto mt-4 max-w-md text-[0.95rem] text-muted-foreground">
        Cette sélection est en cours de préparation. Revenez bientôt pour de nouvelles idées malines.
      </p>
    </div>
  );
}